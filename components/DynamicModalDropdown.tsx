...
interface NavigationItem {
  label: string;         // Button label to be displayed
  screenName: string;    // Screen name to navigate to
}

interface DynamicModalDropdownProps {
  visible: boolean;             // Visibility state of the modal
  onClose: () => void;          // Function to close the modal
  screens: NavigationItem[];    // Array of objects with `label` and `screenName`
...
const DynamicModalDropdown: React.FC<DynamicModalDropdownProps> = ({ visible, onClose, screens, title = "Navigate to" }) => {
  ...
  const handleOptionPress = (screenName: string) => {
    ...
    navigation.navigate(screenName as never); // Then navigate to the selected screen
  };
